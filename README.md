...
この時、zipになっているgatlingを使う場合、libraryを置くフォルダに自分でコピーする必要があります。
この場合、依存関係をしっかり把握していないと、何をコピーするのか分からず、大変です。
推移依存がある場合などを考えた時に、手元でパッケージングするのが正解だと思いました。

そのため、sbt-packを利用して負荷試験のシナリオ群を１つにパッケージして使っています。
...
負荷試験をしている環境とビルド周りの話を少し記事にしてみました。
負荷試験をやっていきましょう。
